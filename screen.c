...
// in this file we are going to define a nember of functions for screen
//manipulation. These functuins include erase screen, set color attributes,
//set cursor location, etc...using VT100 escape sequences.
//move between files: alt_., alt_,
//follow this rwference: http://www.termsys.demon.co.uk/vtansi.htm
//use SHIFT_insert key to pasts
...
void drawbar(int col, int height){
	...
	for(i=1; i<=height; i++){
	  gotoXY(35-i, col);
#ifdef UNICODE		//following codes are in conditional compliation
		printf("%s", BAR);
#else
		 printf("%c", '#');
#endif
...
Position getscreensize(void){
	Position p;
	char ret[100] = "\0";		//make an empty string for quary return
	int r, c;
